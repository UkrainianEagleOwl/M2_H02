...
def play():
    ...
    # player.fill((WHITE))
    player_imgs = [
        pygame.image.load(IMGS_PATH_GOOSE + file).convert_alpha()
        for file in listdir(IMGS_PATH_GOOSE)
    ]
    ...
    bg = pygame.transform.scale(
        pygame.image.load(IMGS_PATH_GAME + "background2.png").convert(), screen
    )
    bgX = 0
    bgX2 = bg.get_width()
    bg_speed = 5
    ...
    def show_game_over_screen():
        ...
        while game_over:
            ...
            custom_image = pygame.image.load(
                IMGS_PATH_GAME + "goose_doose.jpg"
            ).convert_alpha()
    ...
    while is_working:
        ...
        main_surface.blit(
            font.render(str(scores) + " score", True, BLACK), (width - 110, 0)
        )
        main_surface.blit(
            font.render(str(bombs) + " bombs", True, BLACK), (width - 110, 20)
        )
        main_surface.blit(
            font.render(str(lives) + " lives", True, BLACK), (width - 110, 40)
        )
            ...
            if player_rect.colliderect(enemy[1]):
                ...
                if lives == 0:
                    pygame.time.set_timer(
                        DEATH_EVENT, 10
                    )  # Показати екран програшу через 0 секунд
        ...
        # Керування
        if (
            pressed_keys[K_DOWN] or pressed_keys[K_s]
        ) and not player_rect.bottom >= height:
        ...
        if (
            pressed_keys[K_RIGHT] or pressed_keys[K_d]
        ) and not player_rect.right >= width:
