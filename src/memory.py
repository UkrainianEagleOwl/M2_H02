...
class AddressBook(UserDict):
            ...
            if user_data.user_birthday.value:
                ...
                if current_time < user_birthday <= max_date:
                    ...
                    user_info = {
                        ...
                        "user_phones": ", ".join(
                            [str(x) for x in user_data.user_phones]
                        ),
                        "user_birthday": user_data.user_birthday.value.strftime(
                            "%d %B"
                        ),
                        "day": "🎂" + " " + greeting_day,
                    }
    ...
    def to_dict(self):
        ...
        return {"data": {name: record.to_dict() for name, record in self.data.items()}}

    def from_dict(cls, data):
        # get address book object from dictionary represantation
        address_book = cls()
        records = [
            Record.from_dict(record_data) for record_data in data["data"].values()
        ]
...
class Phone(Field):
    ...
    def value(self, new_value):
        ...
        if not isinstance(new_value, str):
            try:
                ...
                raise SetterValueIncorrect(
                    Fore.RED
                    + "Incorrect type of information for phone number."
                    + Style.RESET_ALL
                )
        ...
        if re.search(r"^\+?[0-9]{1,4}\s?[0-9]{4,14}$", new_value):
            ...
            raise_mesage = (
                Fore.RED
                + "You write phone number incorrectly. \n"
                + Style.RESET_ALL
                + "Number must begin with plus, and be without spaces,hyphen or brackets. Example: "
                + Fore.BLUE
                + '"+380990658131"'
                + Style.RESET_ALL
            )
        ...
        if new_value is None:
            ...
            if isinstance(new_value, datetime):
                ...
                try:
                    ...
                    raise_mesage = (
                        Fore.RED
                        + "You write birthday data incorrectly. \n"
                        + Style.RESET_ALL
                        + "Format of birthday data is day/month/year. Example: "
                        + Fore.BLUE
                        + '"28/06/2023"'
                        + Style.RESET_ALL
                    )
                ...
                raise SetterValueIncorrect(
                    Fore.RED + "Only string data or datetime accepted" + Style.RESET_ALL
                )
        ...
        if new_value == None:
            ...
            if not isinstance(new_value, str):
                try:
                    ...
                    raise SetterValueIncorrect(
                        Fore.RED
                        + "Incorrect type of information for email."
                        + Style.RESET_ALL
                    )
            ...
            if re.search(r"^[\w\.-]+@[\w\.-]+\.\w+$", new_value):
                ...
                raise_mesage = (
                    Fore.RED
                    + "You write email incorrectly. \n"
                    + Style.RESET_ALL
                    + "Here is a template for email: "
                    + Fore.BLUE
                    + '"test@example.com"'
                    + Style.RESET_ALL
                )
        ...
        if new_value == None:
            ...
            if not isinstance(new_value, str):
                try:
                    ...
                    raise SetterValueIncorrect(
                        Fore.RED
                        + "Incorrect type of information for address."
                        + Style.RESET_ALL
                    )
            ...
            if re.search(r"^\w+\s+\d+(?:,\s+\w+)?(?:,\s+\w+)?$", new_value):
                ...
                raise_mesage = (
                    Fore.RED
                    + "You write address incorrectly. \n"
                    + Style.RESET_ALL
                    + "The correct entry of the address is the street, number, city and, if desired, the country. Here is a template: "
                    + Fore.BLUE
                    + '"Potyomkinskya 30, Kyiv"'
                    + Style.RESET_ALL
                )
                raise SetterValueIncorrect(raise_mesage)
...
class Record:
    ...
    def __init__(self, name, phone=None, birthday=None, email=None, address=None):
        ...
        self.user_name = (
            ...
            else self.check_argument_for_init(name, "name")
        )
        self.user_phones = []
        self.user_birthday = (
            birthday
            if isinstance(birthday, Birthday)
            else self.check_argument_for_init(birthday, "birthday")
        )
        self.user_email = (
            email
            if isinstance(email, Email)
            else self.check_argument_for_init(email, "email")
        )
        self.user_address = (
            address
            if isinstance(address, Address)
            else self.check_argument_for_init(address, "address")
        )
    ...
    def to_dict(self):
        ...
        return {
            ...
            "user_birthday": self.user_birthday.value.strftime("%d/%m/%Y")
            if self.user_birthday.value
            else None,
    ...
    def from_dict(cls, data):
        ...
        record = Record(
            data_name, birthday=data_birthday, email=data_email, address=data_address
        )
        for phone in phones:
            record.add_phone(phone)
        return record
    ...
    def change_record_info(self, change_field, new_info):
        if change_field == "phone":
            self.user_phones[0].value = (
                new_info
                if isinstance(new_info, Phone)
                else self.check_argument_for_init(new_info, "phone")
            )
        elif change_field == "email":
            self.user_email = (
                new_info
                if isinstance(new_info, Email)
                else self.check_argument_for_init(new_info, "email")
            )
        elif change_field == "birthday":
            self.user_birthday = (
                new_info
                if isinstance(new_info, Birthday)
                else self.check_argument_for_init(new_info, "birthday")
            )
        elif change_field == "address":
            self.user_address = (
                new_info
                if isinstance(new_info, Address)
                else self.check_argument_for_init(new_info, "address")
            )
    ...
    def days_to_birthday(self):
        ...
        this_year_birthday = datetime(
            ...
            month=self.user_birthday.value.month,
            day=self.user_birthday.value.day,
        )
